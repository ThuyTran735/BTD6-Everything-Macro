...
HandleDefeat() {
    ...
    if !GameStatePatterns.Has("Restart") {
        ToolTip("Restart pattern is missing.")
        return false
    }


    firstRestart := GameStatePatterns["Restart"]
    ...
    if !FindText(
        ...
        ToolTip("Could not find first Restart button.")
        return false
    }
    ...
    if GameStatePatterns.Has("Restart Confirm") {
        confirmPattern := GameStatePatterns["Restart Confirm"]
